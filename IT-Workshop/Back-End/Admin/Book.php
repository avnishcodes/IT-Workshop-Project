        ...
        <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <div class="hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-2">
                <ul class="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                    ...
                    <li>
                        <a href="#" class="block py-2 pr-4 pl-3 text-white rounded bg-primary-700 lg:bg-transparent lg:text-primary-700 lg:p-0 dark:text-white" aria-current="page">Books</a>
                    </li>
                    <li>
...
<section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <button type="button" class=" bg-sky-600 flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                       
                        <a href="http://localhost/IT%20Workshop/Back-End/Admin/Add%20books/index.php" class="block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-primary-700 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700">
    ...
    <table class="w-full text-sm text-left text-white-500 dark:text-gray-400">
        ...
        $sql = "SELECT Title, Author, Publisher, Publication_Year, Genre, Language, 
                       Number_of_Copies, Location, Description, Status, Cover_Image FROM books";

        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            echo '<thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">';
            echo '<tr>';
            echo '<th scope="col" class="px-4 py-3">Title</th>';
            echo '<th scope="col" class="px-4 py-3">Author</th>';
            echo '<th scope="col" class="px-4 py-3">Publisher</th>';
            echo '<th scope="col" class="px-4 py-3">Publication Year</th>';
            echo '<th scope="col" class="px-4 py-3">Genre</th>';
            echo '<th scope="col" class="px-4 py-3">Language</th>';
            echo '<th scope="col" class="px-4 py-3">Number of Copies</th>';
            echo '<th scope="col" class="px-4 py-3">Location</th>';
            echo '<th scope="col" class="px-4 py-3">Description</th>';
            echo '<th scope="col" class="px-4 py-3">Status</th>';
            echo '<th scope="col" class="px-4 py-3">Cover Image</th>';
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td class="px-4 py-3">' . $row['Title'] . '</td>';
                echo '<td class="px-4 py-3">' . $row['Author'] . '</td>';
                echo '<td class="px-4 py-3">' . $row['Publisher'] . '</td>';
                echo '<td class="px-4 py-3">' . $row['Publication_Year'] . '</td>';
                echo '<td class="px-4 py-3">' . $row['Genre'] . '</td>';
                echo '<td class="px-4 py-3">' . $row['Language'] . '</td>';
                echo '<td class="px-4 py-3">' . $row['Number_of_Copies'] . '</td>';
                ...
                echo '<td class="px-4 py-3">' . $truncatedDescription . ' <a href="#" class="text-blue-600 hover:underline" onclick="alert(\'' . addslashes($description) . '\')">Read More</a></td>';

                echo '<td class="px-4 py-3">' . $row['Status'] . '</td>';
                echo '<td class="px-4 py-3">';
                if (!empty($row['Cover_Image'])) {
                    echo '<img src="http://localhost/IT%20Workshop/Back-End/Admin/Add%20books/cover/' . $row['Cover_Image'] . '" alt="Cover" width="100">';
                } else {
                    echo 'No Cover Image';
                }
                echo '</td>';
